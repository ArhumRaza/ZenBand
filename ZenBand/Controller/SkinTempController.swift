...
class SkinTempController: UIViewController {
    ...
    let anxietyLevel: UILabel = {
        ...
        label.textColor = UIColor(red:1.000, green:0.800, blue:0.200, alpha:1)
    ...
    let middleLabel: UITextView = {
        let label = UITextView()
        label.textColor = UIColor(red:1.000, green:0.800, blue:0.200, alpha:1)
        label.text = "Skin Temperature Threshold:"
        label.font = UIFont(name:"American Typewriter", size: 20)
        label.translatesAutoresizingMaskIntoConstraints = false
    ...
    let middleLabel2: UITextView = {
        let label = UITextView()
        label.textColor = UIColor(red:1.000, green:0.800, blue:0.200, alpha:1)
        label.text = "Average Skin Temperature:"
        label.font = UIFont(name:"American Typewriter", size: 20)
        label.translatesAutoresizingMaskIntoConstraints = false
        label.backgroundColor = UIColor(red:1, green:1, blue:0.941, alpha:1)
        return label
...
//          view.addSubview(calibrateButton)
    ...
    func fetchData() {
        ...
        ref.child("Values").observe(.childAdded, with: { (snapshot) in
            ...
            self.sensor.setTemp(temp: (dictionary!["Skin temperature"] as? Double)!)
            self.sensor.setTime(time: (dictionary!["Time"] as? Int)!)
            self.sensor.setReset(reset: (dictionary!["Sensor Reset"] as? Int)!)
            self.sensor.setThreshold(threshold: (dictionary!["Temp threshold"] as? Double)!)
            ...
            if self.numbers.count == 0 {
                ...
                let tempAvg = (Double(tempCntr))/(Double(self.numbers.count))
                self.midLabelValue2.text = String(round(10*tempAvg)/10)
            }
            ...
            if  self.sensor.getTemp() > tempLow {
                temp =  self.sensor.getTemp()
                
                self.numbers.append(Double(self.sensor.getTemp()))
                ...
                if temp > tempLow {
                    self.numbers.append(Double(temp))
                    self.time.append(Double(cntr))
                    self.updateGraph()
                }
                print("Value was nil")
            ...
            print("Skin Temp: ", self.sensor.getTemp() as Any, "\tTime: ",  self.sensor.getTime() as Any, "\tReset: ",  self.sensor.isReset() as Any)
